...
if (slider) {
  ...
  slider.addEventListener('touchend', e => {
    ...
    if (Math.abs(diff) > 40) {
      diff > 0
        ? nextT()
        : showT((curT - 1 + tCards.length) % tCards.length);
      resetTimer();
    }
  });
}
...
function validateForm() {
  ...
  if (!fn.value.trim())                    { setFieldError('fn',  'fnE',  true);  valid = false; } else setFieldError('fn',  'fnE',  false);
  if (!ln.value.trim())                    { setFieldError('ln',  'lnE',  true);  valid = false; } else setFieldError('ln',  'lnE',  false);
  if (!isValidEmail(em.value.trim()))      { setFieldError('em',  'emE',  true);  valid = false; } else setFieldError('em',  'emE',  false);
  if (!cy.value.trim())                    { setFieldError('cy',  'cyE',  true);  valid = false; } else setFieldError('cy',  'cyE',  false);
  if (!int.value)                          { setFieldError('int', 'intE', true);  valid = false; } else setFieldError('int', 'intE', false);
  if (msg.value.trim().length < 10)        { setFieldError('msg', 'msgE', true);  valid = false; } else setFieldError('msg', 'msgE', false);
